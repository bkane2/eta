...
; two-person games.)
;
; TODO: possibly move the condition checking whether the current step is
; a "^you" step or a "^me" step (or something else) outside of this
; function; into the perform-next-step task.
;
  (let (curr-step subplan wff ret bindings new-subplan)
    ...
    ; Based on WFF of current schema step, either try to match the expected step to an
    ; observation, or execute the current step if it is an action by Eta or an abstract
    ; plan episode (e.g. 'trying until').
    (cond

      ; If expected user action
      ((eq (car wff) '^you)
        ; Instantiate the episode variable of the new step and substitute
        (instantiate-curr-step subplan)
      ...
      ; If expected Eta action or abstract plan episode
      (t
        ; Instantiate the episode variable of the new step and substitute
        (instantiate-curr-step subplan)
        ...
        (setq ret (if (eq (car wff) '^me)
          ...
          (implement-next-plan-episode subplan)))
