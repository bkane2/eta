...
(defparameter *redirect-to-options*

'(event-schema :header (((set-of ^me ^you) redirect-to-options.v) ** ?e)
;````````````````````````````````````````````````````````````````````````````````
...
)) ; END defparameter *redirect-to-options*
...
;
(store-schema-name 'redirect-to-options.v '*redirect-to-options*)
...
;
(setf (get '*redirect-to-options* 'semantics) (make-hash-table))
(setf (get '*redirect-to-options* 'gist-clauses) (make-hash-table))
(setf (get '*redirect-to-options* 'topic-keys) (make-hash-table))
...
;
(mapcar #'(lambda (x)
      (store-output-semantics (first x) (second x) '*redirect-to-options*))
  '()
...
;
(mapcar #'(lambda (x) 
      (store-output-gist-clauses (first x) (second x) '*redirect-to-options*))
  '()
...
;
(mapcar #'(lambda (x) 
      (store-topic-keys (first x) (second x) '*redirect-to-options*))
  '()
