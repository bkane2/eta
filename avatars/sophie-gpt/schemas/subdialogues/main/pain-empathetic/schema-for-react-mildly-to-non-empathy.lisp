...
(defparameter *react-mildly-to-non-empathy*
...
)) ; END defparameter *react-mildly-to-non-empathy*
...
;
(store-schema-name 'react-mildly-to-non-empathy.v '*react-mildly-to-non-empathy*)
...
;
(setf (get '*react-mildly-to-non-empathy* 'semantics) (make-hash-table))
(setf (get '*react-mildly-to-non-empathy* 'gist-clauses) (make-hash-table))
(setf (get '*react-mildly-to-non-empathy* 'topic-keys) (make-hash-table))
...
;
(mapcar #'(lambda (x)
      (store-output-semantics (first x) (second x) '*react-mildly-to-non-empathy*))
  '()
...
;
(mapcar #'(lambda (x) 
      (store-output-gist-clauses (first x) (second x) '*react-mildly-to-non-empathy*))
  '()
...
;
(mapcar #'(lambda (x) 
      (store-topic-keys (first x) (second x) '*react-mildly-to-non-empathy*))
  '()
